...
@Last update  : 2021-09-13 09:40:12
"""
...
class Example(object):
    def __init__(self, question_text, question_aspect, answer_text, answer_aspect, review_texts, review_aspects, review_opinions, ratings, vocab):
        ...
        # 处理问题的属性词
        q_aspect_words = question_aspect.split()
        if len(q_aspect_words) > 0:
            self.que_asp = [vocab.word2id(w) for w in q_aspect_words]
            self.que_asp_len = len(q_aspect_words)
        else:
            self.que_asp = [self.que_input[0]]
            self.que_asp_len = 1
        ...
        # 处理答案的属性词
        a_aspect_words = answer_aspect.split()
        if len(a_aspect_words) > 0:
            self.ans_asp = [vocab.word2id(w) for w in a_aspect_words]
            self.ans_asp = self.ans_asp[:5]
            self.ans_asp_len = len(a_aspect_words)
        else:
            self.ans_asp = [self.dec_input[0]]
            self.ans_asp_len = 1
        ...
        reviews_words = []
        for review, rev_asp, rev_opi in zip(review_texts, review_aspects, review_opinions):
            ...
            r_aspect_words = rev_asp.split()
            r_opinion_words = rev_opi.split()
            if len(r_aspect_words) > 5:
                r_aspect_words = r_aspect_words[:5]
                r_opinion_words = r_opinion_words[:5]
            elif len(r_aspect_words) == 0:
                r_aspect_words = [review_words[0]]
                r_opinion_words = [review_words[0]]
            self.rev_asp.append([vocab.word2id(w) for w in r_aspect_words])
            self.rev_opi.append([vocab.word2id(w) for w in r_opinion_words])
            self.rev_asp_len.append(len(r_aspect_words))
        ...
        self.original_reviews = review_texts
    ...
    def pad_aspect_opinion(self, q_asp_len, a_asp_len, r_asp_len, pad_id):
        while len(self.que_asp) < q_asp_len:
            self.que_asp.append(pad_id)
        while len(self.ans_asp) < a_asp_len:
            self.ans_asp.append(pad_id)
        for i in range(len(self.rev_asp)):
            while len(self.rev_asp[i]) < r_asp_len:
                self.rev_asp[i].append(pad_id)
            while len(self.rev_opi[i]) < r_asp_len:
                self.rev_opi[i].append(pad_id)
...
class Batch(object):
    ...
    def init_encoder_seq(self, example_list):
        ...
        for ex in example_list:
            ...
            ex.pad_aspect_opinion(q_asp_len=3, a_asp_len=5, r_asp_len=5, pad_id=self.pad_id)
        ...
        self.que_mask = np.zeros((self.batch_size, max_que_seq_len), dtype=np.float32)
        self.que_asp = np.zeros((self.batch_size, 3), dtype=np.int32)
        self.que_asp_mask = np.zeros((self.batch_size, 3), dtype=np.int32)
        ...
        self.rev_mask = np.zeros((self.batch_size, config.review_num, max_rev_seq_len), dtype=np.float32)
        self.rev_asp = np.zeros((self.batch_size, config.review_num, 5), dtype=np.int32)
        self.rev_opi = np.zeros((self.batch_size, config.review_num, 5), dtype=np.int32)
        self.rev_asp_mask = np.zeros((self.batch_size, config.review_num, 5), dtype=np.int32)
        ...
        for i, ex in enumerate(example_list):
            ...
            self.que_asp[i] = ex.que_asp

            self.rev_batch[i] = ex.rev_inputs
            self.rev_lens[i] = ex.rev_lens
            self.rev_asp[i] = ex.rev_asp
            self.rev_opi[i] = ex.rev_opi
            self.rating_batch[i] = ex.rating_inputs

            for j in range(ex.que_len):
                self.que_mask[i][j] = 1

            for j in range(ex.que_asp_len):
                self.que_asp_mask[i][j] = 1

            for j in range(len(ex.rev_lens)):
                for k in range(ex.rev_lens[j]):
                    self.rev_mask[i][j][k] = 1
                for k in range(ex.rev_asp_len[j]):
                    self.rev_asp_mask[i][j][k] = 1
        ...
        if config.pointer_gen:
            ...
            # 考虑OOV词的编码器输入序列
            self.que_extend_vocab = np.zeros((self.batch_size, max_que_seq_len), dtype=np.int32)
            self.rev_extend_vocab = np.zeros(
                (self.batch_size, config.review_num, max_rev_seq_len), dtype=np.int32)

            for i, ex in enumerate(example_list):
                self.que_extend_vocab[i, :] = ex.que_input_extend_vocab[:]

            for i, ex in enumerate(example_list):
                self.rev_extend_vocab[i, :] = ex.rev_inputs_extend_vocab[:]
    ...
    def init_decoder_seq(self, example_list):
        ...
        self.ans_asp = np.zeros((self.batch_size, 5), dtype=np.int32)
        self.ans_asp_len = np.zeros((self.batch_size, 1), dtype=np.int32)
        self.dec_batch = np.zeros((self.batch_size, config.max_dec_steps), dtype=np.int32)
        self.target_batch = np.zeros((self.batch_size, config.max_dec_steps), dtype=np.int32)
        self.dec_mask = np.zeros((self.batch_size, config.max_dec_steps), dtype=np.float32)
        self.dec_lens = np.zeros((self.batch_size), dtype=np.int32)

        for i, ex in enumerate(example_list):
            self.ans_asp[i] = ex.ans_asp
            self.ans_asp_len[i] = ex.ans_asp_len
...
def get_input_from_batch(batch):
    ...
    que_mask = torch.tensor(batch.que_mask, dtype=torch.float, device=config.DEVICE)
    que_lens = torch.tensor(batch.que_lens, dtype=torch.long)
    que_asp = torch.tensor(batch.que_asp, dtype=torch.long, device=config.DEVICE)
    que_asp_mask = torch.tensor(batch.que_asp_mask, dtype=torch.float, device=config.DEVICE)

    rev_batch = torch.tensor(batch.rev_batch, dtype=torch.long, device=config.DEVICE)
    rev_mask = torch.tensor(batch.rev_mask, dtype=torch.float, device=config.DEVICE)
    rev_lens = torch.tensor(batch.rev_lens, dtype=torch.long)
    rev_asp = torch.tensor(batch.rev_asp, dtype=torch.long, device=config.DEVICE)
    rev_opi = torch.tensor(batch.rev_opi, dtype=torch.long, device=config.DEVICE)
    rev_asp_mask = torch.tensor(batch.rev_asp_mask, dtype=torch.float, device=config.DEVICE)
    rating_batch = torch.tensor(batch.rating_batch, dtype=torch.long, device=config.DEVICE)

    extra_zeros = None
    que_extend_vocab = None
    rev_extend_vocab = None

    if config.pointer_gen:
        que_extend_vocab = torch.tensor(batch.que_extend_vocab, dtype=torch.long, device=config.DEVICE)
        rev_extend_vocab = torch.tensor(batch.rev_extend_vocab, dtype=torch.long, device=config.DEVICE)
    ...
    if config.is_coverage:
        qr_cov = torch.zeros((config.batch_size, que_batch.size(1) + config.topk * rev_batch.size(2)), device=config.DEVICE)

    return que_batch, que_mask, que_lens, que_extend_vocab, que_asp, que_asp_mask, rev_batch, rev_mask, rev_lens, rev_extend_vocab, rev_asp, rev_opi, rev_asp_mask, extra_zeros, rating_batch, c_t_0, qr_cov


def get_output_from_batch(batch):
    ans_asp = torch.tensor(batch.ans_asp, dtype=torch.long, device=config.DEVICE)
    ans_asp_len = torch.tensor(batch.ans_asp_len, dtype=torch.long, device=config.DEVICE)
    dec_batch = torch.tensor(batch.dec_batch, dtype=torch.long, device=config.DEVICE)
    dec_mask = torch.tensor(batch.dec_mask, dtype=torch.float, device=config.DEVICE)
    dec_lens = torch.tensor(batch.dec_lens, dtype=torch.int64, device=config.DEVICE)

    max_dec_len = np.max(batch.dec_lens)
    target_batch = torch.tensor(batch.target_batch, dtype=torch.long, device=config.DEVICE)

    return dec_batch, dec_mask, max_dec_len, dec_lens, target_batch, ans_asp, ans_asp_len
...
def get_batch_data_list(data_path, vocab, batch_size, mode):
    ...
    for f in filelist:
        ...
        while True:
            ...
            question_aspect = e.features.feature["question_aspect"].bytes_list.value[0].decode().strip()
            answer_text = e.features.feature["answer"].bytes_list.value[0].decode().strip()
            answer_aspect = e.features.feature["answer_aspect"].bytes_list.value[0].decode().strip()
            review_texts = eval(e.features.feature["reviews"].bytes_list.value[0].decode().strip())
            review_aspects = eval(e.features.feature["review_aspects"].bytes_list.value[0].decode().strip())
            review_opinions = eval(e.features.feature["review_opinions"].bytes_list.value[0].decode().strip())
            ...
            example = Example(question_text, question_aspect, answer_text, answer_aspect, review_texts, review_aspects, review_opinions, ratings, vocab)
...
def get_init_embeddings(reversed_dict):
    ...
    for _, word in sorted(reversed_dict.items()):
        try:
            ...
            word_vec = np.random.normal(0, 1, config.emb_dim)
    ...
    return torch.tensor(word_vec_list, dtype=torch.float)
